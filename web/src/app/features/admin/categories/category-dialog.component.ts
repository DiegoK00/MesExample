...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ isEdit ? 'Modifica Categoria' : 'Nuova Categoria' }}</h2>
...
export class CategoryDialogComponent implements OnInit {
  ...
  submit(): void {
    ...
    const call = this.isEdit
      ? this.categoriesService.update(this.data.category!.id, { name: v.name, description: v.description || undefined })
      : this.categoriesService.create({ name: v.name, description: v.description || undefined });
